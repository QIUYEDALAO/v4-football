...
function buyTiming(oddsSignal, oddsCur) {
  ...
  if (oddsSignal.direction === 'down') {
    ...
    return { timing: '✅直接投', detail: `降盘至${cur}，门槛降低` };
  }
...
function extractH2H(text) {
  ...
  for (const line of text.substring(s, e).split('\n')) {
    ...
    const type = p[0] || '';
    const yr = parseInt(((p[1] || '').match(/(\d{4})/i) || ['0'])[1]);
    if (isFriendly(type) || yr < 2020) continue;
...
// 格式：半场|主胜|和|客胜|让球上盘|盘口|让球下盘|大小大球水|**大小球盘口**|大小小球水
function extractCrownOdds(text) {
  ...
  for (const line of lines) {
    ...
    if (!inCrown) continue;
    // 找到半场行
    if (!t.startsWith('半场\t') && !t.startsWith('半　场\t')) continue;
    const parts = t.split('\t').filter(x => x.trim());
    if (parts.length < 9) continue;
    const goalLine = parts[8];
    if (!goalLine || goalLine === '-') continue;
    let val = null;
    if (goalLine.includes('/')) {
      const p = goalLine.split('/');
      val = (parseFloat(p[0]) + parseFloat(p[1])) / 2;
    } else {
      val = parseFloat(goalLine);
    }
    if (val !== null && !isNaN(val)) return { init: val, cur: val };
  }
  // 回退：找全场大小球
  inCrown = false;
  for (const line of lines) {
...
function smartRecommend(h2h, recent, odds, teamStats) {
  ...
  const oddsSig = analyzeOdds(odds);
  const recentSig = analyzeRecentSignal(recent);
  const timing = buyTiming(oddsSig, odds?.cur);
  ...
  // 盘口≥1.5 → 用团队数据判断
  if (timing.timing === '⏳尝试') {
    const oddsVal = normalizeOdds(odds?.cur);
  ...
  // ===== 初盘匹配度 =====
  const fit = oddsDataFit(odds?.init, h2h.weightedAvg);
  ...
  const totalBonus = streakBonus + fitBonus + teamBonus + lateBias;
