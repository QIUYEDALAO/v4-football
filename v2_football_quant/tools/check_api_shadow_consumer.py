...
DRYRUN_SCHEMA = "api_shadow_consumer_dryrun.v1"
ALLOWED = {"dashboard", "replay", "audit"}
MUST_BLOCK = {"v2_daily_pool", "v2_window_checker", "v2_settlement", "v4_scan", "v4_review", "qq_sender"}
...
def _source_code_checks() -> tuple[bool, list[str]]:
    p = BASE_DIR / "engine" / "api_shadow_consumer.py"
    if not p.exists():
        return False, ["shadow_consumer_source_missing"]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Check non-critical shadow consumer dry-run marker")
    parser.add_argument("--date", required=True, help="YYYYMMDD")
    args = parser.parse_args()
    date_key = args.date.strip().replace("-", "")

    in_path = STATUS_DIR / f"api_shadow_consumer_dryrun_{date_key}.json"
    out_path = STATUS_DIR / f"api_shadow_consumer_check_{date_key}.json"
    ...
    if not in_path.exists():
        result = {
            ...
            "warnings": warnings,
            "errors": ["shadow_consumer_dryrun_marker_missing"],
            "date": date_key,
    ...
    boundary_valid = True
    if str(marker.get("mode", "")).lower() != "non_critical_shadow":
        boundary_valid = False
        errors.append("mode_invalid")
    for k in ["no_api", "no_key_read", "no_push", "no_strategy_recompute", "no_cron", "production_path_untouched", "fallback_to_original_source"]:
    ...
    allowed = set(marker.get("allowed_consumers", []) if isinstance(marker.get("allowed_consumers", []), list) else [])
    blocked = set(marker.get("blocked_consumers", []) if isinstance(marker.get("blocked_consumers", []), list) else [])
    consumer_scope_valid = True
    if not ALLOWED.issubset(allowed):
        consumer_scope_valid = False
        errors.append("allowed_consumers_missing_required")
    if not MUST_BLOCK.issubset(blocked):
        consumer_scope_valid = False
        errors.append("blocked_consumers_missing_required")
    # hard stop if production consumers leak into allowed list
    forbidden_in_allowed = MUST_BLOCK.intersection(allowed)
    if forbidden_in_allowed:
        consumer_scope_valid = False
        errors.append("forbidden_consumers_in_allowed")
    ...
    status = "PASS"
    if not schema_valid or not boundary_valid or not consumer_scope_valid or not secret_safe:
        status = "FAIL"
    ...
    result = {
        ...
        "consumer_scope_valid": consumer_scope_valid,
