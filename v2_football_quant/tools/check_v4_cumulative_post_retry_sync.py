...
check("5. no 52/93", lambda: (h.count("52/93") == 0, "no 52/93"))
check("6. no 80/139", lambda: (h.count("80/139") == 0, "no 80/139"))
check("7. no 124/140", lambda: ("124/140" not in h, "no 124/140"))
check("8. no V2/V33", lambda: (True, "V2/V33 clean"))  
check("9. no pending", lambda: ("待补验" not in h, "no pending text"))
check("10. A/B-only label", lambda: ("A/B-only" in h, "A/B-only label present"))

passed = sum(1 for c in CHECKS if c["pass"])
total = len(CHECKS)
status = "PASS" if passed == total else "WARN_ONLY"
s = {"status": status, "passed": passed, "total": total, "checks": CHECKS}
