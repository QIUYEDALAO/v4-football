#!/usr/bin/env python3
"""Phase D.8.37 — formal state write path proof plan (plan-only)."""
...
SCHEMA_VERSION = "v2_formal_state_write_path_proof_plan.v1"
...
def _base(date_key: str, window: str, status: str) -> dict[str, Any]:
    return {
        ...
        "date": date_key,
        "window": window,
        "formal_state_write_path_proof_plan_status": status,
...
def main() -> None:
    ...
    p_d836 = STATUS_DIR / f"v2_production_verified_write_path_proof_plan_{date_key}_{window}.json"
    ...
    sources = {"d836": d836, "d835": d835, "d831": d831, "d830": d830}
    ...
    if blockers:
        ...
        out.update(
            {
                ...
                "proof_plan": {
                    "must_keep_formal_state_write_disabled": True,
                    "must_not_write_selected_fixtures": True,
                    "must_not_write_official_bet_locked": True,
                    "must_not_write_settlement_required": True,
                    "must_not_write_qq_required": True,
                },
                "production_gates": {
                    "production_resume_allowed_now": False,
                    "cron_enable_allowed": False,
        ...
        out_path = STATUS_DIR / f"v2_formal_state_write_path_proof_plan_{date_key}_{window}.json"
    ...
    if str(d836.get("production_verified_write_path_proof_plan_status", "")) not in {"READY_FOR_BOSS_REVIEW", "WARN"}:
        blockers.append("D836_STATUS_INVALID")
    ...
    d838 = d836.get("d838_draft", {}) if isinstance(d836.get("d838_draft"), dict) else {}
    if _bool(d838.get("allowed_to_execute"), True):
        blockers.append("D838_ALLOWED_TO_EXECUTE_TRUE")

    warnings.append("FORMAL_STATE_WRITE_PATH_UNPROVEN")
    ...
    out.update(
        {
            ...
            "d836_status": d836.get("production_verified_write_path_proof_plan_status"),
            "proof_plan": {
                "must_keep_formal_state_write_disabled": True,
                "must_not_write_selected_fixtures": True,
                "must_not_write_official_bet_locked": True,
