...
DRYRUN_SCHEMA = "api_aux_explain_dryrun.v1"
...
ALLOWED_LABEL_TOKENS = ["辅助解释", "不作生产证据", "非生产证据", "正式链路禁用cache"]
FORBIDDEN_WORDING = [
    "V2已通过cache",
    "V4已通过cache",
    "已接入生产",
...
FORBIDDEN_RESPONSE_KEYS = {"raw_response", "response_body", "body", "full_response"}
...
def _has_forbidden_response_key(obj: Any) -> bool:
    if isinstance(obj, dict):
        for k, v in obj.items():
            if str(k).lower() in FORBIDDEN_RESPONSE_KEYS:
                return True
...
def _source_code_checks() -> tuple[bool, list[str]]:
    src = BASE_DIR / "engine" / "api_aux_explain.py"
    if not src.exists():
        return False, ["aux_explain_source_missing"]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Check API auxiliary explanation dry-run marker")
    parser.add_argument("--date", required=True, help="YYYYMMDD")
    args = parser.parse_args()

    date_key = args.date.strip().replace("-", "")
    in_path = STATUS_DIR / f"api_aux_explain_dryrun_{date_key}.json"
    out_path = STATUS_DIR / f"api_aux_explain_check_{date_key}.json"
    ...
    if not in_path.exists():
        result = {
            ...
            "warnings": [],
            "errors": ["aux_explain_dryrun_marker_missing"],
            "date": date_key,
    ...
    boundary_valid = True
    if str(marker.get("mode", "")).lower() != "auxiliary_explanation":
        boundary_valid = False
    ...
    all_text_parts.extend([str(x) for x in capabilities])
    all_text_parts.extend([str(x) for x in limitations])
    ...
    for phrase in FORBIDDEN_WORDING:
        if phrase in all_text:
            wording_valid = False
            errors.append(f"forbidden_wording_detected:{phrase}")

    if int(marker.get("capabilities_count", 0) or 0) <= 0:
        wording_valid = False
        errors.append("capabilities_count_not_positive")
    if int(marker.get("limitations_count", 0) or 0) <= 0:
        wording_valid = False
        errors.append("limitations_count_not_positive")
    if int(marker.get("explanation_cards_count", 0) or 0) <= 0:
        wording_valid = False
        errors.append("explanation_cards_count_not_positive")
    ...
    status = "PASS"
    if not schema_valid or not boundary_valid or not display_scope_valid or not labels_valid or not wording_valid or not secret_safe:
        status = "FAIL"
