...
def main():
    ...
    if rops_model.get('match_display')!='Rops vs OLS（原始队名）': blockers.append(f'rops_model_match_display_missing:{rops_model.get("match_display")}')
    ...
    if 'B级候选' not in html or 'Rops vs OLS（原始队名）' not in html: blockers.append('html_rops_b_candidate_display_missing')
    ...
    if '数据缺口：</span><b>进球分布不可用</b>' not in html:
    ...
    for full_text in ['Rops vs OLS（原始队名）','芬甲 / Finland Ykkonen','暂无真实进球分布','原因：H2H样本不足 / 联赛长期样本不足 / 数据源未返回进球时间分布','不支持原因：</span><b>H2H样本不足 / 联赛长期样本不足 / 数据源未返回进球时间分布</b>']:
    ...
    out={'checker':'tools/check_v3v4_dashboard_after_scan_refresh.py','phase':'V3V4-DASHBOARD-DYNAMIC-DATE-MARKER-AND-MATCHDATE-TZ-HOTFIX','date':DATE,'conclusion':'PASS' if not blockers else 'BLOCKER','after_scan_time':data.get('planned_time'),'requires_scan_completed':data.get('requires_scan_completed'),'scan_completed':data.get('scan_completed'),'brief_ready':data.get('brief_ready'),'candidate_ready':data.get('candidate_ready'),'validation_preserved':data.get('validation_preserved'),'validation_touched':data.get('validation_touched'),'result_validation_changed':data.get('result_validation_changed'),'script_validation_changed':data.get('script_validation_changed'),'status':data.get('status'),'candidate_counts':{'A':candidate.get('A_count'),'B':candidate.get('B_count'),'C':candidate.get('C_count'),'SKIP':candidate.get('SKIP_count'),'scan_total':candidate.get('scan_total')},'B_candidates':[{'fixture_id':x.get('fixture_id'),'home':x.get('home'),'away':x.get('away'),'grade':x.get('grade')} for x in b_candidates if isinstance(x,dict)],'dashboard_model_counts':{'A':model_today.get('A'),'B':model_today.get('B'),'SKIP':model_today.get('SKIP'),'scan_total':model_today.get('scan_total')},'dashboard_model_candidate_source':(model.get('data_sources') or {}).get('candidates'),'rops_model_display':{k:rops_model.get(k) for k in ['match_display','league_display','goal_distribution_status','goal_distribution_missing_reason','data_gap_display','unsupported_reason']},'dashboard_build_counts':{'A':build.get('A'),'B':build.get('B'),'C':build.get('C_deprecated_count'),'SKIP':build.get('SKIP'),'scan_total':build.get('scan_total')},'dashboard_build_candidate_source':build.get('candidate_source'),'html_display_checks':{'rops_original_visible':'Rops vs OLS（原始队名）' in html,'league_alias_visible':'芬甲 / Finland Ykkonen' in html,'goal_distribution_missing_visible':'暂无真实进球分布' in html,'missing_reason_visible':'数据源未返回进球时间分布' in html,'right_data_gap_full_visible':'数据缺口：</span><b>进球分布不可用</b>' in html,'candidate_before_validation':0 <= pos_candidate < pos_validation,'right_decision_before_v3':0 <= pos_decision < pos_v3},'fallback_used':candidate.get('fallback_used'),'fallback_reason':candidate.get('fallback_reason'),'fallback_overwrites_effective_scan_result':False if int(candidate.get('B_count',0) or 0)>0 else bool(candidate.get('fallback_used')),'dynamic_marker_guard':not blockers,'blockers':blockers}
