...
## 📅 每周审判日复盘制度 (Weekly Audit Protocol)

**执行频率**: 每周一上午（或无赛事的周中）。
**唯一操作**: 运行 `python engine/paper_trading.py --summary`，并审查 Router 日志。

**核心审判指标 (The Holy Trinity)**:
1. **黄金区 (Boost 候选)**: `[5] -> [4]` 等微小跳变区的 `N` 是否逼近/突破 20？其 `True CLV` 是否稳健为正？
2. **毒药区 (Blacklist 候选)**: `[5] -> [3]` 等大幅跳变区的 `True CLV` 是否持续深陷负值？
3. **Router 拦截率**: 查看日志中 `BLOCK_ROUTER_ACTIVATION` 出现的频次。如果频繁因为 `CLV <= 0` 被拦截，说明该因子存在严重的周期性衰退，绝不可盲目解封。

---

## 🌉 Phase 4: 纸盘至实盘的"过桥策略" (Bridge to Live Spec)

*当前状态：Draft（仅规范，不写代码。待纸盘满 50 场后评估是否启动）*

在纸盘证明了 Alpha 的存在后，绝不能直接全仓全垒打。必须经过严格的"小仓试水（Small-Stake Live）"阶段，以检验**"实盘滑点"、"封盘拒单"**以及**"心态扭曲"**。

### ⛩️ 第一道闸：实盘准入条件 (The Live Gates)

必须**同时满足**以下 4 个条件，才允许将系统接入真实资金：
- [ ] **样本厚度**: 纸盘总验证样本 `N ≥ 50` 场。
- [ ] **大盘优势**: 过去 50 场的全局 `avg_true_clv ≥ +1.0%`。
- [ ] **风控健康**: 纸盘资金曲线的最大回撤 `MDD ≤ 12%`。
- [ ] **核心发力**: 黄金跳变区（`attrition_boost_candidate`）的 CLV 必须明确为正，证明战力折损因子在发挥真实作用。

### 🛡️ 试水期运行规则 (Small-Stake Mode)

一旦开启实盘，强制进入"试水模式"至少 2 周或 20 场：
1. **取消 Kelly 波动**: 关闭 1/4 Kelly 动态注码，改用**固定小额注码（Flat Betting）**。每注严格限定为总本金的 `0.5% ~ 1.0%`（例如 20,000 本金，每注死定 100-200 块）。
2. **锁死 Boost 加权**: 即便 Router 算出了高 Priority 和 Boost，**试水期绝不放大仓位**，只跟随方向下注。
3. **控制过曝 (Over-exposure)**: 每日最大实盘下注数强制 `<= 3` 场。优中选优。

### 🛑 试水期熔断退回机制 (The Live Kill-Switch)

实盘交易中，如果最近 20 场实盘订单的滚动去水收盘线价值（`Live rolling_20 avg_true_clv`）**跌破 0%**：
- **无条件切断 API 实盘下单权限。**
- **系统立刻打回"纸盘模式（Paper Trading）"重新反省。**
- 说明实盘遇到了严重的滑点、市场微观结构剧变，或纸盘数据存在过拟合。

---

> 💡 **最后更新**：2026-05-07 23:46  
> **状态**：V2 引擎封板 · Phase 3 防贪婪机制上线 · 纸盘→实盘过桥 Spec 入宪  
> **下一里程碑**：积累 N ≥ 50 · 每周一审判日 · 迎接 Phase 4
